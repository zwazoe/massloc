...
// collections
const { Access, AgeRange, Availability, Booking, Credential, DateRange, Days, Description, Discount, FileManagement, Gallery, Lifespan, Grouping, Pricing, Product, ProductName, QuantityRange, Question, SizeRange, Strategy, TimeRange, Variation, Warranty, Wholesale } = require('../../../models/things');
...
const Collections = {
	// products
	Access, AgeRange, Availability, Booking, Credential, DateRange,  Days, Description, Discount, FileManagement, Gallery, Lifespan, Grouping, Pricing, Product, ProductName, QuantityRange, Question, SizeRange, Strategy, Section, TimeRange, Variation, Warranty, Wholesale,
	// places
	Business,
...
class CRUD {
	...
	CreateItem(req, res, Collection, collection, payload, noBusiness = [], profile = null, place = null) {
		...
		const mad = new MAD(
			...
			['field'], //// demarel fields which will mix and match it.
			{
				...
				no_lower_case: ['attributes_employer_association', 'attributes_place_association']
			}
		);
		const data = mad.run();
		// console.log( payload, 'payload from brain')
		// console.log( data, 'payload from brain')
								...
								this.CreateItem(req, res, Collection, collection, req.body, noBusiness, profile.id, place)
									.then(created_products => {
										...
										Section.find({ _id: { $in: created_products.data[0].attributes_section } }).then(sections => {
											...
											if(sections){
												sections.map(section => {
													...
													created_products.data.map(created_product => {
														if( section_field && !section_field.includes(created_product._id)){
											...
											console.log(sections, "adding new collection")
	...
	getProductAction() {
		this.router.post('/get-products/:product_id', passport.authenticate('jwt', { session: false }), (req, res) => {
			...
			let promise = (item = null) =>  new Promise(resolve => {
				resolve(item)
			})

			promise(product_id).then(_id => {
				if(_id!=="undefined"){
	...
	TheReadItems(collection, user_id) {
		...
		if (noProfile.includes(collection)) {
			...
			return Profile.findOne({
				...
				data.map(attributes => {
					Section.find({id: {$in: attributes.attributes_section}})
				})
				return Section.find().then(sections => {
					console.log('getting sections', sections)
					return { data, sections}
				})
			}).then(payloads => {
				payloads.data.map(data => {
					data.attributes_section.map(attributes_section => {
						console.log('testing payloads', payloads, data, payload.sections[attributes_section])
					})
				})
				// console.log(payloads, 'the payload' ) 
				return new Promise((resolve, reject) => {
					resolve(payloads.data)
				});
			}).then(data => {
				let payloads = data
				// console.log(payloads, 'the payload' ) 
	...
	ReadItemsAuto() {
		...
		this.router.get('/all/auto', passport.authenticate('jwt', { session: false }), (req, res) => {
			...
			this.TheReadItems('Access', req.user.id)
